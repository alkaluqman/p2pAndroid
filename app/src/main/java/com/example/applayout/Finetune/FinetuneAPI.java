...
public class FinetuneAPI {
     ...
     */
    public static void finetune(String modelFileAbsolutePath, String datasetDirAbsolutePath, int numEpochs, int batchSize) {
        Log.d("FinetuneAPI", "In FinetuneAPI.finetune()");
        try {
            new FinetuneTask(modelFileAbsolutePath, datasetDirAbsolutePath, numEpochs, batchSize).execute().get();
